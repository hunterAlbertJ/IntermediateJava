...
public class InMemoryCatalog implements Catalog {
    ...
    public Collection<MusicItem> findByKeyword(String keyword) {
        //declare return value, in this case an empty result collection
        Collection<MusicItem> result = new ArrayList<>();
          for(MusicItem item :catalogData){
              if(item.getTitle().toString().equals(keyword)){
...
/**
     ...
     */
    public Collection<MusicItem>  maxRockValue(double price){
         Collection<MusicItem> result = new ArrayList<>();
         for(MusicItem item : catalogData){
             if(item.getMusicCategory() == MusicCategory.ROCK && item.getPrice() <= price){
                 result.add(item);
             }
         }
